...
public interface DepartmentRepository extends JpaRepository<Department, Long> {
    ...
    @Query(value = "SELECT c.department_id FROM courses c where c.course_id = :courseId", nativeQuery = true)
    ...
    @Query(value = "SELECT STRING_AGG(DISTINCT c.semester::text, ',' ORDER BY c.semester) AS semesters " +
                "FROM departments d " +
                "JOIN courses c ON d.department_id = c.department_id " +
                "JOIN questions q ON c.course_id = q.course_id " +
